...
export default function Algebra1DiagnosticPage() {
  ...
  const [showTimer, setShowTimer] = useState<boolean>(true); // Timer visibility toggle
  const [elapsedTime, setElapsedTime] = useState<number>(0); // Elapsed time in seconds
  const [questionTimes, setQuestionTimes] = useState<{questionId: string, timeSpent: number}[]>([]); // Track time per question
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl sm:text-3xl font-bold text-orange-600">{formatTime(totalTime)}</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          >
            ...
            <div className="mb-4 p-4 bg-orange-50 rounded-lg border border-orange-200">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-orange-600">{formatTime(avgTimePerQuestion)}</span>
              </div>
            </div>
              ...
              {questionTimes.map((qt, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-gray-900 w-16 text-right">{formatTime(qt.timeSpent)}</span>
                  </div>
                </div>
              ))}
            </div>
