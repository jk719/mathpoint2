...
export function GradeSelectorSimple() {
  ...
  return (
    <div className="relative p-6 sm:p-8 md:p-10 bg-gradient-to-br from-white via-gray-50 to-orange-50 rounded-2xl shadow-2xl overflow-hidden">
      ...
      <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-8 relative">
        <span className="bg-gradient-to-r from-[#1a3a52] to-[#ff6b35] bg-clip-text text-transparent">
          Choose Your Grade Level
        </span>
        <div className="mt-2 text-xs sm:text-sm font-normal text-gray-600">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2 sm:gap-3 relative z-10">
        {[6, 7, 8, 9, 10, 11, 12].map((grade) => (
          <button
            key={grade}
            className={`group relative py-4 sm:py-6 px-2 sm:px-4 rounded-lg sm:rounded-xl transition-all duration-300 transform hover:-translate-y-1 ${
              selectedGrade === grade
          ...
          >
            {selectedGrade === grade && (
              <div className="absolute inset-0 bg-gradient-to-br from-[#ff6b35] to-[#ff8c5f] rounded-lg sm:rounded-xl blur opacity-50" />
            )}
            <div className="relative">
              ...
              <div className={`text-[10px] sm:text-xs font-semibold mt-1 leading-tight ${
      ...
      {selectedGrade && (
        <div className="mt-6 sm:mt-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-[#1a3a52] to-[#0f2436] text-white rounded-full">
            <svg className="animate-spin h-4 w-4 sm:h-5 sm:w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span className="font-semibold text-sm sm:text-base">Preparing Grade {selectedGrade} Assessment...</span>
          </div>
        </div>
      )}
    </div>
